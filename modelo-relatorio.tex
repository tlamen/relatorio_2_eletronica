...
Esta seção descreve os procedimentos adotados na simulação e na bancada, com os parâmetros que permitem repetir cada medida. A Seção~4.1 apresenta a simulação de projeto de cada integrante; a Seção~4.2 descreve a montagem em bancada; a Seção~4.3 trata da simulação da montagem com os valores medidos; e a Seção~4.4 resume os instrumentos e as condições de medida.

Todas as simulações foram feitas no LTspice, com os símbolos construídos na Seção~2.1 do roteiro: o LM741 (arquivo \texttt{item\_6.asy}) nas Seções~2.2, 2.3 e 2.4, e o TL064 (arquivo \texttt{item\_7.asy}) na Seção~2.5. A biblioteca \texttt{ene0046.lib} foi mantida na mesma pasta dos esquemáticos, e o atributo \texttt{ModelFile} dos símbolos aponta para ela pelo nome, sem caminho. Cada arquivo traz uma única diretiva de análise ativa, nós nomeados de forma descritiva e diretivas gravadas como \textit{SPICE directive}. O nome e a matrícula de cada integrante estão anotados no desenho como comentário.

\subsection{Simulação de projeto}
...
\subsubsection{Seguidor de tensão (\texttt{item\_1.asc})}
...
\subsubsection{Não inversor (\texttt{item\_2.asc})}

O circuito é o amplificador não inversor com o LM741, com $R_f$ e $R_g$ projetados para o ganho $G$ da matrícula. A análise \texttt{.tran} foi feita com entrada senoidal de \SI{1}{\kilo\hertz}, em duas amplitudes de pico: uma que deixa a saída a pelo menos \SI{2}{\volt} da saturação e outra que leva a saída à saturação. As duas condições foram obtidas por varredura paramétrica (\texttt{.step}) da amplitude.

\subsubsection{Somador inversor (\texttt{item\_3.asc} e \texttt{item\_4.asc})}

O circuito é o amplificador inversor com o LM741 e duas entradas contínuas independentes, $V_1$ e $V_2$. Em \texttt{item\_3.asc} roda-se uma análise \texttt{.op} com os valores escolhidos de $V_1$ e $V_2$, de modo que a saída fique a pelo menos \SI{2}{\volt} da saturação. Em \texttt{item\_4.asc} roda-se uma varredura \texttt{.dc} de $V_1$ de \SI{0}{\volt} a \SI{1}{\volt}, com $V_2$ fixo no valor escolhido.

> **[INSERIR: valores de $V_1$ e $V_2$ escolhidos, passo da varredura \texttt{.dc} e valores de $R_1$, $R_2$ e $R_f$ por integrante.]**

\subsubsection{Amplificador de diferença (\texttt{item\_5.asc})}

O circuito é o amplificador de diferença de quatro resistores, com o TL064. As duas condições de ensaio são dois circuitos independentes desenhados no mesmo arquivo: no ensaio diferencial, a fonte senoidal entra por $R_1$ e a outra entrada vai ao terra; no ensaio de modo comum, a mesma fonte alimenta as duas entradas. A análise é \texttt{.tran} em \SI{1}{\kilo\hertz} com \SI{100}{\milli\volt} de pico.

> **[INSERIR: tempo de simulação, passo de integração e valores de $R_1$, $R_2$, $R_3$ e $R_4$ por integrante.]**
...
Montou-se o amplificador de diferença com o TL064, com os quatro resistores casados dois a dois, de modo a realizar o ganho diferencial $A_d = 2{,}14$ da especificação da equipe~2. No ensaio diferencial, aplicou-se uma senoide de \SI{1}{\kilo\hertz} e \SI{100}{\milli\volt} de pico numa entrada e a referência contínua $V_{ref} = \SI{0,5}{\volt}$ na outra. No ensaio de modo comum, ligaram-se as duas entradas juntas ao gerador, com \SI{1}{\kilo\hertz} e \SI{5}{\volt} de pico. A montagem está na Figura~\ref{fig:dif_bancada}, e os valores medidos estão na Tabela~\ref{tab:dif_bancada}.

> **[INSERIR: valores dos quatro resistores medidos, tensão de saída no ensaio diferencial, tensão de saída no ensaio de modo comum e ganho de modo comum calculado. As imagens das duas condições de ensaio entram como figuras.]**
...
\hline
$R_1 = R_3$ & [INSERIR] & [INSERIR] \\
$R_2 = R_4$ & [INSERIR] & [INSERIR] \\
$V_{in}$ (pico) & \SI{100}{\milli\volt} & \SI{5}{\volt} \\
$V_{ref}$ & \SI{0,5}{\volt} & --- \\
$V_o$ (pico) & [INSERIR] & [INSERIR] \\
$A_d$ realizado & [INSERIR] & --- \\
$A_{cm}$ & --- & [INSERIR] \\
\hline
\end{tabular}
\end{table}

Os modelos dos instrumentos usados na bancada estão na Tabela~\ref{tab:instrumentos}, e as incertezas de cada escala estão registradas junto às leituras na Seção~5.

\begin{table}[h]
\centering
\caption{Instrumentos usados na montagem.}
\label{tab:instrumentos}
\begin{tabular}{lll}
\hline
Instrumento & Modelo & Função \\
\hline
Fonte de bancada & [INSERIR] & \SI{\pm 15}{\volt} e \SI{6}{\volt} contínuo \\
Gerador de ondas & [INSERIR] & Senoides de \SI{1}{\kilo\hertz} \\
Osciloscópio & [INSERIR] & Leitura de amplitudes \\
Multímetro & [INSERIR] & Medição de resistores e tensões contínuas \\
\hline
\end{tabular}
\end{table}
...
Refez-se a simulação usando os valores medidos dos resistores que estavam na bancada, com os mesmos arquivos e as mesmas diretivas de análise da Seção~4.1, alterando apenas os valores dos componentes. Essa simulação não substitui a de projeto: é a comparação entre as duas que separa o efeito da tolerância dos componentes do restante.

> **[INSERIR: quais arquivos foram reaproveitados, quais valores medidos foram substituídos e se alguma condição de análise precisou ser ajustada.]**

\subsection{Instrumentos e condições de medida}

A Tabela~\ref{tab:instrumentos} resume os instrumentos usados e as condições de medida. Para cada leitura de bancada, a incerteza do instrumento é a da escala utilizada.

> **[INSERIR: modelos dos instrumentos, conforme os do laboratório, e incertezas das escalas usadas.]**
